...
# Usage: MIMO_RELAY_WS_URL=ws://gateway:8000/ws bash scripts/deploy.sh
...
for pkg in websockets httpx; do
  if ! python3 -c "import $pkg" 2>/dev/null; then
    echo "    Installing $pkg..."
    pip install -q --break-system-packages $pkg 2>/dev/null || pip3 install -q --break-system-packages $pkg 2>/dev/null
  fi
done
echo "    Dependencies OK"

# ── 2. Health check: skip if already running ──
...
# ── 3. Kill old bridge processes ──
echo "==> Cleaning up old processes..."
pkill -f "bridge.py" 2>/dev/null || true
sleep 1

# ── 4. Start bridge in background ──
...
cd "$SKILL_DIR"
nohup python3 "$BRIDGE_SCRIPT" "$MIMO_RELAY_WS_URL" > "$LOG_FILE" 2>&1 &
BRIDGE_PID=$!
